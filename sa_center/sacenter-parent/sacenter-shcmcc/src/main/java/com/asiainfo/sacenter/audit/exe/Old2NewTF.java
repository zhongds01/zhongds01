...
public class Old2NewTF implements ITransform {
    private static Log log = LogFactory.getLog(Old2NewTF.class);
    ...
    public void transform(HashMap[] maps) throws Exception {
        if (maps != null && maps.length > 0) {
            for (int i = 0; i < maps.length; i++) {
                ...
                Map iopenProvisionOld = maps[i];
                String DONE_CODE_NEW = (String) iopenProvisionOld.get("DONE_CODE");
                String PS_SERVICE_TYPE_NEW = (String) iopenProvisionOld.get("PS_SERVICE_TYPE");
                String BILL_ID_NEW = (String) iopenProvisionOld.get("BILL_ID");
                String SUB_BILL_ID_NEW = (String) iopenProvisionOld.get("SUB_BILL_ID");
                Long ACTION_ID_NEW = null == iopenProvisionOld.get("ACTION_ID") ? 0L : Long.parseLong(iopenProvisionOld.get("ACTION_ID") + "");
                int STOP_TYPE_NEW = null == iopenProvisionOld.get("STOP_TYPE") ? 0 : Integer.parseInt(iopenProvisionOld.get("STOP_TYPE") + "");

                if("1000".equals(PS_SERVICE_TYPE_NEW))
                {
                    PS_SERVICE_TYPE_NEW="HLR";
                }else
                {
                    IYSSV iyssv = (IYSSV) ServiceFactory.getService(IYSSV.class);
                    IBOYSValue[] ts=iyssv.queryByActionId(ACTION_ID_NEW);
                    if(null!=ts&&ts.length>0)
                    {
                        PS_SERVICE_TYPE_NEW=ts[0].getPsServiceType();
                    }
                }

                String OLD_PS_PARAM_NEW = (String) iopenProvisionOld.get("OLD_PS_PARAM");
                splintString(sourceDataMap, Old2NewTF.OLD_PS_PARAM_PREFIX, OLD_PS_PARAM_NEW, 1, sourceKeyList);
                String PS_PARAM_NEW = (String) iopenProvisionOld.get("PS_PARAM");
                splintString(sourceDataMap, Old2NewTF.PS_PARAM_PREFIX, PS_PARAM_NEW, 1, sourceKeyList);
                ...
                ISaEquipMentNewSV iSaEquipMentNewSV = (ISaEquipMentNewSV) ServiceFactory.getService(ISaEquipMentNewSV.class);
                IBOISAEQUIPMENTNEWValue[] iboisaequipmentnewValues = iSaEquipMentNewSV.queryByDoneCode(DONE_CODE_NEW);
                if (null != iboisaequipmentnewValues && 1 == iboisaequipmentnewValues.length) {
                    String DONE_CODE_OLD = iboisaequipmentnewValues[0].getDoneCode();
                    String PS_SERVICE_TYPE_OLD = iboisaequipmentnewValues[0].getPsServiceType();
                    String BILL_ID_OLD = iboisaequipmentnewValues[0].getBillId();
                    String SUB_BILL_ID_OLD = iboisaequipmentnewValues[0].getSubBillId();
                    Long ACTION_ID_OLD = iboisaequipmentnewValues[0].getActionId();
                    int STOP_TYPE_OLD = iboisaequipmentnewValues[0].getStopType();

                    String OLD_PS_PARAM_OLD = iboisaequipmentnewValues[0].getOldPsParam();
                    splintString(targetDataMap, Old2NewTF.OLD_PS_PARAM_PREFIX, OLD_PS_PARAM_OLD, 0, targetKeyList);

                    String PS_PARAM_OLD = iboisaequipmentnewValues[0].getPsParam();
                    splintString(targetDataMap, Old2NewTF.PS_PARAM_PREFIX, PS_PARAM_OLD, 0, targetKeyList);
    ...
    public void saveResult(List errorData, int diff_type) {
        String sql = "insert into COMPARE_RESULT_OLD2NEW (done_code, diff_type,key_name,new_value,old_value) values (?,?,?,?,?)";
    ...
    private void saveSingle(String done_code, int diff_type) {
        String sql = "insert into COMPARE_RESULT_OLD2NEW (done_code, diff_type) values (" + done_code + "," + diff_type + ")";
        Connection conn = null;
        PreparedStatement insertStmt = null;
        try {
            SFCenterFactory.pushCenterInfo("200");
    ...
    public static void main(String[] args) throws Exception {
        try {
            TfFrameWork.main(new String[]{"I_OPEN_PROVISION_OLD", "1", "0"});
        } catch (Exception e) {
            log.error(e);
            throw e;
        }
